...
async def _pre_examine(handler: HandlerMetaData, event: MessageEvent) -> bool:
    """过期检查"""
    if handler.expire_time and datetime.now() > handler.expire_time:
        HandleRegistry.unregister(
    ...
    if accepted_adapters := handler.plugin.adapters \
        and event.adapter_cls not in accepted_adapters:
        return False
    
    """规则检查"""
    for rule in handler.rules:
        ctn = await rule.check(event) \
            if asyncio.iscoroutinefunction(rule.check) \
            else rule.check(event)
        if not ctn:
            return False
