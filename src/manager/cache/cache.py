...
class Cache(ABC):
    ...
    async def _get(self, cache_key: str) -> any:
        """获取Cache信息"""
        pickled_data =  await self._redis.get(key=cache_key)
    ...
    async def _set(self, cache_key: str, cache_data: any) -> None:
        ...
        return await self._redis.set(key=cache_key, value=pickled_data, ex=self.cache_ttl)
    
        
    async def _remove(self, cache_key: str) -> None:
        """移除指定群聊的缓存"""
        await self._redis.delete(key=cache_key)
    ...
    async def _exists(self, cache_key: str) -> bool:
        """判断缓存是否存在"""
        try:
            return await self._redis.exists(cache_key)
